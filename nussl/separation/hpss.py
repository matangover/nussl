...
class HPSS(mask_separation_base.MaskSeparationBase):
    ...
    def __init__(self, input_audio_signal, kernel_size=31,
        ...
        super(HPSS, self).__init__(input_audio_signal=input_audio_signal)
        self.harmonic = None
    ...
    def run(self):
        ...
        for i in range(self.audio_signal.num_channels):
            # apply mask
            harmonic_mask, percussive_mask = librosa.decompose.hpss(self.stft[:, :, i], kernel_size=self.kernel_size, mask=True)
